...
function ProductCard({ product }) {
  return <button className="product-card" onClick={() => goTo(`product/${product.slug}`)}><span className="product-image"><img src={product.image} alt="" /></span><span className="product-meta"><small>{product.group}</small><strong>{product.name}</strong><span>{product.status}<ArrowRight size={15} /></span></span></button>;
}
...
function ProductsPage() {
  ...
  return <><PageHero eyebrow="Complete source catalog" title="Products for every part of the stay." summary="Browse SDC’s original hospitality catalog in one clean, searchable place. Product pricing and final specifications are confirmed by quotation." count={`${catalog.length} products`} /><section className="catalog-section"><div className="catalog-toolbar"><label className="search-box"><MagnifyingGlass size={20} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search products" /></label><select aria-label="Filter by collection" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All collections</option>{catalogCategories.map((item) => <option key={item.slug} value={item.slug}>{item.name} ({item.count})</option>)}</select></div><div className="category-directory">{catalogCategories.map((item) => <button className={category === item.slug ? "selected" : ""} onClick={() => setCategory(item.slug)} key={item.slug}><span>{item.name}</span><small>{item.count}</small></button>)}</div><div className="results-line"><strong>{filteredProducts.length} results</strong><span>Information reconstructed from the original SDC website</span></div>{filteredProducts.length ? <div className="product-grid">{filteredProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="empty-state"><Package size={38} /><h3>No matching products</h3><p>Try another search or choose a different collection.</p></div>}</section><SpecialCollections /></>;
}
...
function EventsPage() {
  return <><PageHero eyebrow="Industry archive" title="Events, training and community." summary="A complete view of the exhibitions, product launches, competitions, partnerships and housekeeping training listed by SDC." count={`${eventArchive.length} events`} /><section className="archive-grid">{eventArchive.map((event, index) => <article key={event.title}><img src={event.image} alt="" /><div><span>{String(index + 1).padStart(2, "0")}</span><small>{event.type}</small><h3>{event.title}</h3></div></article>)}</section></>;
}

...
function LanguageSwitch() {
  ...
  return <div className="language-switch" role="group" aria-label={copy.language.label}><button className={language === "en" ? "selected" : ""} onClick={() => setLanguage("en")} aria-pressed={language === "en"}>{copy.language.english}</button><button className={language === "km" ? "selected" : ""} onClick={() => setLanguage("km")} aria-pressed={language === "km"}>{copy.language.khmer}</button></div>;
}
...
function EventsPage() {
  ...
  return <><PageHero eyebrow={copy.events.eyebrow} title={copy.events.title} summary={copy.events.summary} count={`${eventArchive.length} events`} /><section className="event-archive"><article className="featured-event"><img src={featuredEvent.image} alt={featuredEvent.title} /><div><span>01</span><p className="eyebrow light">{copy.events.featured} · {featuredEvent.type}</p><h2>{featuredEvent.title}</h2><p>{copy.events.supporting}</p></div></article><div className="archive-grid">{otherEvents.map((event, index) => <article key={event.title}><span className="archive-image"><img src={event.image} alt={event.title} loading="lazy" decoding="async" /><b>{String(index + 2).padStart(2, "0")}</b></span><div><small>{event.type}</small><h3>{event.title}</h3></div></article>)}</div></section></>;
}
